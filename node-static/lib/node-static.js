...
var Server = function(root, options) {
    ...
    this.cacheRule = options.cacheRule || {};

    this.defaultHeaders = {};

    this.options.headers = options.headers || {};

    this.options.indexFile = options.indexFile || "index.html";
    ...
    this.defaultHeaders['Server'] = 'Node-Static/' + this.version;

    var defaultHeaders = this.defaultHeaders;

    for (var k in defaultHeaders) {
        if (defaultHeaders.hasOwnProperty(k)) {
            this.options.headers[k] = this.options.headers[k] || defaultHeaders[k];
        }
    }
};
...
Server.prototype.version = version.join('.');

Server.prototype.triggerWriteHeadBefore = function(status, headers, request, response){
	var event = {
		type: 'writeheadbefore',
		status: status,
		headers: headers,
		request: request,
		response: response,
...
Server.prototype.serveDir = function(pathname, req, res, finish) {
    ...
    function streamFiles(files) {
        Server.mstat(pathname, files, function(e, stat) {
            if (e) {
                return finish(404, {})
            }
...
Server.mstat = function(dir, files, callback) {
    ...
    if (!status || status >= 400) {
        if (callback) {
            ...
            if (promise.listeners('error').length > 0) {
				...
				*+++++++++++++++++++++add by wuhao++++++++++++++++++
				*/
                this.triggerWriteHeadBefore(status, headers, req, res);
				/*
				*---------------------add by wuhao------------------
				*/
...
Server.prototype.respondNoGzip = function(pathname, status, contentType, _headers, files, stat, req, res, finish) {
    ...
    var defaultHeaders = this.options.headers;
    ...
    headers[EtagKey] = JSON.stringify([stat.ino, stat.size, mtime].join('-'));
    ...
    if ((clientMTime || clientETag) &&
        (!clientETag || clientETag === headers[EtagKey]) &&
...
Server.prototype.respond = function(pathname, status, _headers, files, stat, req, res, finish) {
    var contentType = _headers['Content-Type'] ||
        mime.lookup(files[0]) || 'application/octet-stream';
